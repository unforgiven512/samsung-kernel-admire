...
extern struct platform_device keypad_device_escape_REV02;
...
static int msm_fb_lcdc_power_save(int on)
{
	...
	for (i = 0; i < ARRAY_SIZE(msm_fb_lcdc_vreg); i++) {
		if (on) {
            ...
            if (i==0){   
#if defined(CONFIG_MACH_VINO) || defined(CONFIG_MACH_RANT3)
...
#if 0 //MBjkseo 20110321 : After BT off, sleep current is 3mA high. because do not excute function below.
...
#if defined(CONFIG_MACH_RANT3) ||defined(CONFIG_MACH_VINO) || defined(CONFIG_MACH_ROOKIE) || defined(CONFIG_MACH_ESCAPE) || defined(CONFIG_MACH_GIO)
...
#elif defined(CONFIG_ESCAPE_GPIO_KEYPAD)
    platform_device_register(hw_version >= 3 ? &keypad_device_escape : &keypad_device_escape_REV02);
#elif defined(CONFIG_GIO_GPIO_KEYPAD)
  if( hw_version >= 3 )
