...
#endif
#if defined(CONFIG_MACH_RANT3) || defined(CONFIG_MACH_VINO) || defined(CONFIG_MACH_REALITY2)
...
unsigned int fg_read_soc(void)
{
	...
	{
		if (data[0]) //MBjdyoo 2011.06.29 : Display 1 when SOC is 0< SOC <1 (H/W Request)
		{
			...
			adj_soc = (((data[0]*100 + (data[1]>>6)*25 ) - 45) * 100) / (9700 - 45);  //MBjdyoo 2011.06.29 : Change MAX 9480 -> 9700 (H/W Request)
#elif defined(CONFIG_MACH_GIO)
			adj_soc = (((data[0]*100 + (data[1]>>6)*25 ) - 45) * 100) / (9480 - 45); // using the Rookie's algorithm, temporarily
//			adj_soc = (((data[0]*10 + (data[1]>>7)*5 ) - 4) * 100) / (937 - 4); // GIO 1350mA battery
#else
...
gRealsoc = (data[0]*100 + (data[1]>>6)*25 )/10;
		...
		{
#if 0 //MBjdyoo 2011.06.29 : Display 1 when SOC is 0< SOC <1 (H/W Request)  #if 1 ->  #if 0
			return 0;
#else
...
#if defined(CONFIG_MACH_RANT3) || defined(CONFIG_MACH_VINO) || defined(CONFIG_MACH_REALITY2) || defined(CONFIG_MACH_ROOKIE) || defined(CONFIG_MACH_ESCAPE) || defined(CONFIG_MACH_GIO)
