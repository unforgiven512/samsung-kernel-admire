		...
		const char *buf, size_t size)
{
	...
	if(strncmp(buf, "UPDATE", 6) == 0 || strncmp(buf, "update", 6) == 0) {
		...
		if((melfas_mcs7000_ts->fw_ver != 0x23 && melfas_mcs7000_ts->hw_rev == 0x40)
		  ||(melfas_mcs7000_ts->fw_ver != 0x23 && melfas_mcs7000_ts->hw_rev == 0x50))
        { 
...
static DEVICE_ATTR(firmware, S_IRUGO | S_IWUGO, firmware_show_mcs7000, firmware_store_mcs7000);
...
int melfas_mcs7000_ts_probe(void)
{
	...
	if((melfas_mcs7000_ts->fw_ver!=0x11 && melfas_mcs7000_ts->hw_rev==0x12)
        ...
        || (melfas_mcs7000_ts->fw_ver!=0x23 && melfas_mcs7000_ts->hw_rev==0x40)
        || (melfas_mcs7000_ts->fw_ver!=0x23 && melfas_mcs7000_ts->hw_rev==0x50))
...
int melfas_mcs7000_ts_suspend(pm_message_t mesg)
{
  ...
  disable_irq(melfas_mcs7000_ts->irq);
  //flush_workqueue(melfas_mcs7000_ts_wq);
