...
static int cam_hw_init()
{
	...
	gpio_tlmm_config(GPIO_CFG(0, 0,GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE); //RESET
	gpio_tlmm_config(GPIO_CFG(2, 0,GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE); //CAM_EN
	...
	/* Input MCLK = 24MHz */
	gpio_tlmm_config(GPIO_CFG(15, 1,GPIO_OUTPUT, GPIO_NO_PULL, GPIO_16MA), GPIO_ENABLE); //MCLK On
...
static long sr200pc10_set_effect(int mode, int effect)
{
	...
	printk("mode : %d,   effect : %d", mode, effect);
...
static long sr200pc10_set_sensor_mode(int mode)
{
	...
	printk("Sensor Mode  mode : %d", mode);
	switch (mode) {
	case SENSOR_PREVIEW_MODE:
	{
		printk("Preview");
